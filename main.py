...
def getCheaterID():
    ...
    for _ in range(burst):
        ...
        grabbed.append(pyautogui.screenshot(f'{output_folder}{time.strftime("%d-%m.%H-%M-%S")}.{_}.png',
...
def checkNetworks(who):
    ...
    for network in networks:
        ...
        try:
            ...
            addToQueue(f'k d: {kd}, hedshot: {hs}%, level: {lv}')
...
def repeatLast():
    ...
    if last_stats:
        ...
        addToQueue(f'k d: {last_stats[0]}, hedshot: {last_stats[1]}%, level: {last_stats[2]}')
...
# just speak... now brand new threaded and sparkly
def speakToMe():
...
if do_the_speech:
    ...
    threading.Thread(target=speakToMe, daemon=True).start()
